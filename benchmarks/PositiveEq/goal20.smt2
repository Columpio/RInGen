...
(assert (forall ((x Nat)) (ge x Z)))
(assert (forall ((x Nat) (y Nat)) (=> (ge x y) (ge (S x) (S y)))))
...
(assert (forall ((A Lst) (B Nat) (C Lst) (D Nat))
  (=> (and (len C D) (sort C A) (len A B) (= B (S D)))
      incorrect)))
